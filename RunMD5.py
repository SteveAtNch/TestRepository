...
if len(sys.argv) == 1:
    root_dir = 'C:\\Demos\\'    #   VCcode running
else:
...
print("The root folder will be: " + root_dir)
file_set = set()
...
DistinctResults = list(set(results))
print("\r\nDuplicate files: " + str(len(results)-len(DistinctResults)))
print('\nClosing the "OutputCSVfile" with ' + str(index) + ' records.')
print('Execution Time was: ' + str(datetime.datetime.now()-startTime) + '\r\n')
OutputCSVfile.close()
